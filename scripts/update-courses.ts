    ...
    for (const link of uniqueLinks) {
        ...
        try {
            ...
            $header.nextAll('p').each((_, el) => {
                ...
                if (text && !text.startsWith('Academic Career') && !text.startsWith('Course Component')) {
                    description = text;
                    return false; // break
                }
            });
            ...
            const reqMatch = pageText.match(/Course Requirements:\s*([^]*?)(?:Minimum Credits|$)/);
            ...
            const currentSectionsText = $detail('h2:contains("Current Sections")').nextAll().text().toUpperCase();
...
export const CourseInfoData: CourseInfoDataType = ${JSON.stringify(data, null, 2)}
`;
